...
class UnifiedCtimeDiffusion(torch.nn.Module):
    def __init__(
        ...
        # Precompute dummy_mask for _mdlm_update (static across timesteps/batches)
        if len(num_classes) > 0:
            max_K = max(num_classes + 1)
            _mask = torch.zeros(len(num_classes), max_K)
            for i, k in enumerate(num_classes):
                _mask[i, :k + 1] = 1.0
            self.register_buffer('_dummy_mask', _mask)
    ...
    def pad(self, x, pad_value):
        ...
        Returns:
            Tensor: A new tensor with
                    [bs, len(num_classes_w_mask), max(num_classes_w_mask)), num_categories]
        """
        bs = x.shape[0]
        n_cats = len(self.num_classes_w_mask)
        max_K = max(self.num_classes_w_mask)
        out = torch.full((bs, n_cats, max_K), pad_value, dtype=x.dtype, device=x.device)
        offset = 0
        for i, k in enumerate(self.num_classes_w_mask):
            out[:, i, :k] = x[:, offset:offset + k]
            offset += k
        return out
    ...
    def _mdlm_update(self, log_p_x0, x, alpha_t, alpha_s):
        ...
        # _dummy_mask is [n_categories, max_K], broadcasts to [bs, n_categories, max_K]
        q_xs *= self._dummy_mask
