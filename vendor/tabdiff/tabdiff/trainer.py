...
from tabdiff.utils_train import update_ema
...
class Trainer:
    ...
    def run_loop(self):
        ...
        for epoch in range (start_epoch, self.steps):
            if self.max_seconds is not None:
                ...
                if elapsed >= self.max_seconds:
                    print_with_bar(
                        f"Timeout reached: {elapsed / 60:.1f} min elapsed "
            ...
            if total_loss < best_loss and self.curr_epoch > self.check_val_every:
        ...
        # Always save a final checkpoint so short runs aren't lost
        final_state = {
            'denoise_fn': self.diffusion._denoise_fn.state_dict(),
            'num_schedule': self.diffusion.num_schedule.state_dict(),
            'cat_schedule': self.diffusion.cat_schedule.state_dict(),
        }
        torch.save(final_state, os.path.join(self.model_save_path, 'final_model.pt'))
        final_ema_state = {
            'denoise_fn': self.ema_model.state_dict(),
            'num_schedule': self.ema_num_schedule.state_dict(),
            'cat_schedule': self.ema_cat_schedule.state_dict(),
        }
        torch.save(final_ema_state, os.path.join(self.model_save_path, 'final_ema_model.pt'))
