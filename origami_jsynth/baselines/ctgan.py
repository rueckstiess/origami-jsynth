...
class CTGANAdapter:
    ...
    def fit(self, records: list[dict[str, Any]], **kwargs: Any) -> None:
        try:
            ...
            raise ImportError(
                "CTGAN requires the 'sdv' package. "
    ...
    @classmethod
    def load(cls, path: Path, tabular: bool = True) -> CTGANAdapter:
        try:
            from sdv.single_table import CTGANSynthesizer
        except ImportError:
            raise ImportError(
        ...
        adapter._model = CTGANSynthesizer.load(str(path / "model.pkl"))
